...
* **options \(object\)** is een _optionele_ parameter voor opties. Meer info over deze opties is te vinden op de [updateOne\(\)-documenten](https://mongodb.github.io/node-mongodb-native/3.3/api/Collection.html#updateOne) online.
  * `{upsert:true}` een optie om een nieuw document aan te maken wanneer dit nog niet bestaat

```javascript
cursor =  client.db('Les')
          .collection('pokemon')
          .updateOne(
          //enkel als name == charmander (updateOne past enkel eerste aan)
          {name:'charmander'},
          // $set past age aan age naar 2
          {$set:{name:'charmander',age:2}},
          // {upsert:true} maakt een nieuw document aan wanneer het nog niet bestaat
          {upsert:true})
```
...
cursor =  client.db('Les')
          .collection('pokemon')
          .deleteMany(
          //verwijdert alle documenten die overeenkomen met query
          // verwijdert dus alle pikachus
          {name:'pikachu'})
```
...
cursor =  client.db('Les')
          .collection('pokemon')
          .deleteMany(
          //verwijdert alle pokemons met leeftijd groter dan 3
          {age:{$gt:3}})
```

**let op**: Onderstaand voorbeeld maakt heel de DB leeg.

```javascript
let result = await client.db('Les')
        .collection('pokemon')
        .deleteMany({});
```
